...
class EditableImage {
     ...
     * 
     * <p>
     * Export an image to the file that is choosen
     * Also Exports a set of operations from the file with <code>.ops</code> added.
     * So if you Export to <code>some/path/to/image.png</code>, this method will
     * also
     * Export
     * the current operations to <code>some/path/to/image.png.ops</code>.
     * </p>
     ...
     * If Macro has be actived op add to MarcoArray
     * Also snap to image size
     * Clears the shapedOps in imagePanel
     * </p>
     * 
    ...
    public void apply(ImageOperation imageOperation) {
        try {
            if (macro) {
                macroOps.add(imageOperation);
            }

            current = imageOperation.apply(current);
            ops.add(imageOperation);

            Andie.frame.setSize(new Dimension(
                (int) (getCurrentImage().getWidth() * Andie.imagePanel.getZoom() / 100) + 20,
                (int) (getCurrentImage().getHeight() * Andie.imagePanel.getZoom() / 100) + 110));

            Andie.imagePanel.clearOps();
    ...
    /**
     * <p>
     * Save an Macro to file.
     * </p>
     * 
     * <p>
     * Saves an Macro to the choicen file
     * Marco saves a set of operations from the file with <code>.ops</code> added.
     * the current operations to <code>some/path/to/macro.ops</code>.
     * </p>
     * 
     * @throws Exception If something goes wrong.
     */
    public void saveMacro() throws Exception {
        ...
        if (result == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                FileOutputStream fileOut = new FileOutputStream(imageFilepath + ".ops");
                ObjectOutputStream objOut = new ObjectOutputStream(fileOut);
    ...
    public void undo() {
        try {
            ...
            JOptionPane.showMessageDialog(null, Andie.bundle.getString("undoCatch"));
        }
    }
